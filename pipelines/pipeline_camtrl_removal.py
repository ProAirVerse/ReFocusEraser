...
class FluxControlSingleScaleRemovalPipeline(
    ...
    def simulate_zoom_image(
        ...
        Args:
            ...
            zoom (int): zoom focal eg（2, 3, 4）
            output_size (tuple, optional)

        Returns:
            zoomed_image (PIL.Image)
        """
        assert zoom >= 1
    ...
    def check_mask_and_generate_focal_versions(
        ...
        """
        For the given mask, generate 2x/3x/4x zoomed versions, check completeness 
        based on the 1x and 4x mask. If the 4x mask passes the integrity check, 
        generate corresponding zoomed versions of the input image.

        Args:
            mask_pil (PIL.Image): Input mask as a PIL image.
            input_image_pil (PIL.Image): Original input image as a PIL image.
            tolerance (float): Allowed tolerance for completeness check. Default is 0.05.

        Returns:
            Tuple:
                mask_versions (dict): {'1x', '2x', '3x', '4x'} versions of the mask.
                image_versions (dict): Zoomed versions of the input image. {'1x'} if incomplete.
                is_complete (bool): Whether the mask passes the completeness check.
        """
        ...
        # === 1) Generate multi-zoom versions for the mask ===
        mask_1x = mask_pil
        ...
        # === 2) Check completeness based on pixel counts ===
        ...
        # === 3) If complete, generate multi-zoom versions for the input image ===
        if is_complete:
            print("mask_complete....")
            image_versions = {
                "1x": input_image_pil,
    ...
    def prepare_image_with_mask_camera(
        ...
        # Check mask completeness and generate correspoding focal imgs and masks
    ...
    def prepare_image_with_mask_focal(
        ...
        for focal_key in mask_focals.keys():
            ...
            # === Interpolate mask to latent shape ===
            masks = torch.nn.functional.interpolate(
    ...
    def fuse_inverse_zoom_images_with_mask_PIL(
        self,
        image_1x: Image.Image,  # 1x decode 
        image_4x: Image.Image,  # 4x decode 
        mask_1x: Image.Image,   # 4x decode 
        zoom: int = 4,
    ):
        """
        Returns:
